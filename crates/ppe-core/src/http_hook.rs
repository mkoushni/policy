...
crate::define_hooks! {
    ...
    ///
    /// A route selecting on `http:` is matched from the request line under
    /// `engine_settings.dispatch: policy`, which is the default, so a host
    /// that also puts `method` and `path` on the HTTP extension at the
